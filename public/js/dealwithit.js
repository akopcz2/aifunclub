...
if (getQueryVariable('link')) {
    ...
    $('#text').html('');
    scalar = 1;
    $('form').replaceWith('<form action="http://digitallofts.azurewebsites.net/"><input type="submit" id="redirect" value="Wanna make another?"> <input type="button" id="twitter" value="Share on twitter">');
...
$('body').on('click', "#twitter", function () {
    window.open('https://twitter.com/intent/tweet?text=' + encodeURIComponent('Deal with it... ' + encodeURI('http://digitallofts.azurewebsites.net/index.html?link=') + encodeURI(origimgurl)) + '&hashtags=digitallofts');
